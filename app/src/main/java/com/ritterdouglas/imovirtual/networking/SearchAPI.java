...
public interface SearchAPI {
    ...
    Observable<Response<SearchResponse>> search(@Query(LOCATION) String location, @Query(SEARCH_PAGE) String searchPage,
                                                @Query(TYPE) String type, @Query(CAT) String cat, @Query(SIZE_TO) String sizeTo,
                                                @Query(PRICE_FROM) String priceFrom, @Query(PRICE_TO) String priceTo,
                                                @Query(WIDTH) String width, @Query(HEIGHT) String height,
